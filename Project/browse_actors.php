<?php
 ...
 if(!$search) $sql="SELECT * FROM `actors` ORDER BY `list_nr`";
 else $sql="SELECT * FROM `actors` WHERE name LIKE '%$search%' ORDER BY `list_nr`";
 $result=mysqli_query($con,$sql); 
 $total=0;
 if ($result->num_rows > 0)
  while($row = $result->fetch_assoc()) 
  { $total++;
	$movie_table[1][$total] = $row["name"];
	$movie_table[2][$total] = $row["yob"];
	if($movie_table[2][$total]=="") $movie_table[2][$total]="-";
	$movie_table[3][$total] = $row["awards_nr"];
	if($movie_table[3][$total]=="") $movie_table[3][$total]="-";
	$movie_table[4][$total] = $row["uploader"];
	$movie_table[5][$total] = $row["actor_image"];
...
<script type="text/javascript">
 ...
 var actor_name = <?php echo json_encode($movie_table[1]); ?>;
 var actor_yob = <?php echo json_encode($movie_table[2]); ?>;
 var actor_awards = <?php echo json_encode($movie_table[3]); ?>;
 var actor_uploader = <?php echo json_encode($movie_table[4]); ?>;
 var actor_image = <?php echo json_encode($movie_table[5]); ?>;
 var actor_list_nr = <?php echo json_encode($movie_table[6]); ?>;
 ...
 function createTable(page)
  {
	...
	tr.setAttribute("id", "ths");
	 ...
	 var p1_cont = document.createTextNode("Name");
	 p1.appendChild(p1_cont);
	 td1.appendChild(p1);
	 var td2 = document.createElement("th");
	 var p2 = document.createElement("p");
	 var p2_cont = document.createTextNode("Year of birth");
	 p2.appendChild(p2_cont);
	 td2.appendChild(p2);
	 var td3 = document.createElement("th");
	 var p3 = document.createElement("p");
	 var p3_cont = document.createTextNode("Awards");
 ...
 function showPoster(list_nr) 
  {
    if(!actor_image[list_nr]) actor_image[list_nr] = "Undefined.png";
	var image ="url('../Project/thumbnails/" + actor_image[list_nr] + "'), auto";
 ...
 function putNewContent(page)
 {
	...
	for(var i = 1; i <= elements; i++)
	 {
       document.getElementById("p"+i+1).innerHTML = actor_name[(page-1)*10+i];
	   document.getElementById("p"+i+2).innerHTML = actor_yob[(page-1)*10+i];
	   document.getElementById("p"+i+3).innerHTML = actor_awards[(page-1)*10+i];
	   document.getElementById("p"+i+4).innerHTML = actor_uploader[(page-1)*10+i];
	   document.getElementById("p"+i+1).onmouseover = function(index) { showPoster((page - 1) * 10 + index); }.bind(null,i);
	   document.getElementById("p"+i+1).onclick = function(index) { showMovie(actor_list_nr[(page - 1) * 10 + index]); }.bind(null,i);
	 }
 } 
